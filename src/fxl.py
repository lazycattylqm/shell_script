...
def process_file(input_file, output_file, keywords):
  ...
  key_join='|'.join(keywords)
  module_regex = re.compile(r'^\.SUBCKT\s+('+key_join+')\s+')
  ...
  with open(input_file, 'r') as f_in, open(output_file, 'w') as f_out:
    ...
    for line in lines:
      ...
      if module_name and module_name in keywords:
        key_values = re.findall(r'(\w+<?\w*>?)=(\w+<?\w*>?)', line)
  ...
  with open(output_file, 'w') as f:
    ...
    for line in lines:
      ...
      if not in_subckt:
        f.write(line)
      if in_subckt and line.strip().upper().startswith('.ENDS'):
        in_subckt = False
